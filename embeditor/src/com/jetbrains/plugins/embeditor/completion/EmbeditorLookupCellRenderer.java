...
public class EmbeditorLookupCellRenderer implements ListCellRenderer {
  ...
  public EmbeditorLookupCellRenderer(EmbeditorCompletionLookup lookup, EditorColorsScheme scheme) {
    ...
    myNormalMetrics = myLookup.getComponent().getFontMetrics(myNormalFont);
    myBoldMetrics = myLookup.getComponent().getFontMetrics(myBoldFont);
  }
  ...
  public Component getListCellRendererComponent(
    ...
    final LookupElementPresentation presentation = new MyRealLookupElementPresentation(isSelected ? getMaxWidth() : allowedWidth, myNormalMetrics, myBoldMetrics, myLookup);
    if (item.isValid()) {
    ...
    if (isSelected || allowedWidth >= 0) {
      setTailTextLabel(isSelected, presentation, foreground, isSelected ? getMaxWidth() : allowedWidth, false);
    }
  ...
  private void renderItemName(LookupElement item,
    ...
    final String prefix = item instanceof EmptyLookupItem ? "" : ""; //myLookup.itemPattern(item); TODO:???
    if (prefix.length() > 0) {
